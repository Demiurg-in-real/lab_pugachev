...
uint32_t **lik_read(uint32_t *gptstr, uint32_t **massive, uint8_t *file1, uint8_t *number);
...
int zapis(struct form_MBR *Mbr, struct GPT *Gpt, struct razdel *Raz,uint32_t razmer, uint8_t *number,uint8_t *file);//done
...
int main(int argc, unsigned char* argv[])
{
	...
	printf("%i 2\n", razmer123);
	printf("%i he\n", gpt1.signature[0]);
	zapis(&mbr,&gpt1,&raz,razmer123,argv[2],argv[1]);
...
int zapis(struct form_MBR *Mbr, struct GPT *Gpt, struct razdel *Raz,uint32_t razmer, uint8_t *number,uint8_t *file)//so schetom dovedi do uma
{
...
uint32_t **lik_read(uint32_t *gptstr, uint32_t **massive, uint8_t *file1, uint8_t *number1)
{
	uint8_t number=*number1;
